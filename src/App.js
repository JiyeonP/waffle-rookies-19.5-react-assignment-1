...
function App() {
  ...
  const [selectedStd, setSelectedStd] = useState({id:false, name:false, grade:false, profileImg:false});
  const [addStd, setAddStd] = useState(false);
  const [showList, setShowList] = useState(stdList);
  ...
  const searchList = (letter) => {
    setShowList(stdList.filter((item) => item.name.includes(letter)))
  }
